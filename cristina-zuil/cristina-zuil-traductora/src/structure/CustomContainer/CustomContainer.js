...
export function updateContainerVariables() {
  ...
  containers.forEach((container) => {
    ...
    // Calculate margin (when exceeding max-width on lg)
    let margin = 0;
    if (width >= lgBreakpoint && maxWidthValue > 0 && width > maxWidthValue) {
      margin = (width - maxWidthValue) / 2;
    }
    ...
    el.style.setProperty('--container-margin', `${margin}px`);
