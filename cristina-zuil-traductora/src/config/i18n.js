...
export async function fetchPagesByLocale(locale) {
  ...
  const query = new URLSearchParams({
    ...
    'populate[content][on][components.c-title][populate][images][populate]': '*',
    ...
    'populate[content][on][components.c-model][populate]': '*',
  });
...
export async function fetchBooksByLocale(locale, category = null, page = 1, pageSize = 12) {
  ...
  try {
    ...
    console.log(
      `Found ${items.length} books for locale ${locale}${category ? ` in category ${category}` : ''} (total: ${total})`
    );
